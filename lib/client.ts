...
export default class DecoAPIWraper {
  ...
  public async authenticate(password: string): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        'client.ts: ' + `AES Key generated: ${this.aes.key.toString('hex')}`,
      );
      console.log(
        'client.ts: ' + `AES IV generated: ${this.aes.iv.toString('hex')}`,
      );
      ...
      console.log(
        'client.ts: ' + 'Password key retrieved successfully:',
        passwordKey,
      );
  ...
  async getWLAN(): Promise<WLANNetworkResponse | ErrorResponse> {
    ...
    const decodeBase64 = (encoded: string): string => {
      try {
        ...
        console.log(
          'client.ts: ' + `Failed to decode base64 string: ${encoded}`,
          e,
        );
